...
def dashboard_metrics(session) -> dict:
    ...
    # ---------------------------------------------------------
    # Use workflow step durations instead of WorkflowRun.duration_seconds.
    # This avoids counting long human-review waiting periods.
    # ---------------------------------------------------------
    ...
    run_execution_times: dict[str, float] = {}
    ...
    for step in steps:
        ...
        run_execution_times[step.run_id] = (
            run_execution_times.get(
                step.run_id,
                0.0
            )
            + duration
    ...
    def get_latest_run(session) -> Optional[WorkflowRun]:
        return (
            session.query(WorkflowRun)
            .order_by(
    ...
    return {
        ...
        "total_steps_logged": len(all_steps),
    }